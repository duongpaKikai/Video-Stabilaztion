...
class DIFNet2(nn.Module):
	def __init__(self):
		...
		class Backward(torch.nn.Module):
			def __init__(self):
				super(Backward, self).__init__()
				self.tensorGrid = torch.ones([1, 2, 360, 640],dtype=torch.float32, device='cuda:0')
				self.tensorPartial = torch.ones([1, 1, 360, 640],dtype=torch.float32, device='cuda:0')
			# end

			def forward(self, tensorInput, tensorFlow, scale=1.0):
				# if int(self.tensorPartial.size(0)) != int(tensorFlow.size(0)) or int(self.tensorPartial.size(2)) != int(tensorFlow.size(2)) or int(self.tensorPartial.size(3)) != int(tensorFlow.size(3)):
				# self.tensorPartial = torch.FloatTensor().resize_(tensorFlow.size(0), 1, tensorFlow.size(2), tensorFlow.size(3)).fill_(1.0).cuda()
				# end


				# if int(self.tensorGrid.size(0)) != int(tensorFlow.size(0)) or int(self.tensorGrid.size(2)) != int(tensorFlow.size(2)) or int(self.tensorGrid.size(3)) != int(tensorFlow.size(3)):

				tensorHorizontal = torch.linspace(-1.0, 1.0, list_size[3]).view(1, 1, 1, list_size[3]).expand(list_size[0], -1, list_size[2], -1)
				tensorVertical = torch.linspace(-1.0, 1.0, list_size[2]).view(1, 1, list_size[2], 1).expand(list_size[0], -1, -1, list_size[3])

				self.tensorGrid = (torch.cat([ tensorHorizontal, tensorVertical ], 1).cuda()).float()

				# end
				#pdb.set_trace()
				tensorInput = torch.cat([ tensorInput, self.tensorPartial ], 1)
				tensorFlow = torch.cat([ tensorFlow[:, 0:1, :, :] / ((list_size[3] - 1.0) / 2.0), tensorFlow[:, 1:2, :, :] / ((list_size[2] - 1.0) / 2.0) ], 1)
				# print("self.tensorGrid.type(): ",self.tensorGrid.type())
				# print("tensorFlow: ",tensorFlow.type())
	...
	def warpFrame(self, fr_1, fr_2, scale=1.0):
		with torch.no_grad():
			temp_w = int(math.floor(math.ceil(list_size[3] / 64.0) * 64.0)) # Due to Pyramid method?
			temp_h = int(math.floor(math.ceil(list_size[2] / 64.0) * 64.0))

			temp_fr_1 = torch.nn.functional.interpolate(input=fr_1, size=(temp_h, temp_w), mode='nearest')
			temp_fr_2 = torch.nn.functional.interpolate(input=fr_2, size=(temp_h, temp_w), mode='nearest')

			flo = 20.0 * torch.nn.functional.interpolate(input=self.pwc(temp_fr_1, temp_fr_2), size=(list_size[2], list_size[3]), mode='bilinear', align_corners=False)
			# print("flo: ",flo.size())
			# flo_size = [1,2,list_size[2], list_size[3]]
			return self.warpLayer(fr_2, flo, scale), flo

	def forward(self, fr1, fr2, f3, fs2, fs1):
		scale =0.5
