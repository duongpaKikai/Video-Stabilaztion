...
class PwcNet(torch.nn.Module):
	def __init__(self, strModel='sintel'):
		...
		class Backward(torch.nn.Module):
			def __init__(self):
				...
				self.tensorGrid = torch.ones([1, 2, 24, 40],dtype=torch.float32, device='cuda:0')
				self.tensorPartial = torch.ones([1, 1, 24, 40],dtype=torch.float32, device='cuda:0')

			def forward(self, tensorInput, tensorFlow,intL):
				list_size = [1, 2, 96, 160,1, 2, 48, 80,1, 2, 24, 40,1, 2, 12, 20]
				size_0 = list_size[(intL-2)*4]
				size_1 = list_size[(intL-2)*4+1]
				size_2 = list_size[(intL-2)*4+2]
				size_3 = list_size[(intL-2)*4+3]
				# if self.tensorPartial.size(0) != tensorFlow.size(0) or self.tensorPartial.size(2) != tensorFlow.size(2) or self.tensorPartial.size(3) != tensorFlow.size(3):
				self.tensorPartial = torch.ones([size_0, 1, size_2, size_3],dtype=torch.float32, device='cuda:0')
					# print("self.tensorPartial ",self.tensorPartial.size())
				# end

				# if self.tensorGrid.size(0) != tensorFlow.size(0) or self.tensorGrid.size(2) != tensorFlow.size(2) or self.tensorGrid.size(3) != tensorFlow.size(3):
				tensorHorizontal = torch.linspace(-1.0, 1.0, size_3).view(1, 1, 1, size_3).expand(tensorFlow.size(0), -1, size_2, -1)
				tensorVertical = torch.linspace(-1.0, 1.0, size_2).view(1, 1, size_2, 1).expand(tensorFlow.size(0), -1, -1, size_3)

				self.tensorGrid = torch.cat([ tensorHorizontal, tensorVertical ], 1).cuda()
					# print("self.tensorGrid ",self.tensorGrid.size())
				# end

				tensorInput = torch.cat([ tensorInput, self.tensorPartial ], 1)
				tensorFlow = torch.cat([ tensorFlow[:, 0:1, :, :] / ((size_3 - 1.0) / 2.0), tensorFlow[:, 1:2, :, :] / ((size_2 - 1.0) / 2.0) ], 1)
		...
		class Decoder(torch.nn.Module):
			...
			def forward(self, tensorFirst, tensorSecond, objectPrevious):
				...
				tensorVolume = self.moduleCorreleaky(self.moduleCorrelation(tensorFirst, self.moduleBackward(tensorSecond, tensorFlow * self.dblBackward,self.intL),self.intL))
			...
			def forward_2(self, tensorFirst, tensorSecond, objectPrevious):

				tensorVolume = self.moduleCorreleaky(self.moduleCorrelation(tensorFirst, tensorSecond,6))
	...
	def forward(self, tensorFirst, tensorSecond):
		...
		objectEstimate = self.moduleSix.forward_2(tensorFirst[-1], tensorSecond[-1], None)
